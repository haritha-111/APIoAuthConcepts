...
public class Ecommerce {
	...
	public static void main(String[] args) {
		...
		loginRequest.setUserEmail("g.haritha.k@gmail.com");
		loginRequest.setUserPassword("Yash@2114");
		...
		.multiPart("productImage",new File("C:\\Users\\uday2\\OneDrive\\Desktop\\IEvolve_Issue.png"));
		...
		*/
		...
		RequestSpecification ViewOrderReq=	new RequestSpecBuilder().setBaseUri("https://rahulshettyacademy.com")
				...
				RequestSpecification viewOrder =given().log().all().spec(ViewOrderReq).queryParam("key","6553f05e7244490f95e7d248");

				String ViewResponse = viewOrder.when().get("/api/ecom/order/get-orders-for-customer/6553f05e7244490f95e7d248").then().log().all().
				extract().response().asString();

				JsonPath js2 = new JsonPath(ViewResponse);

				Assert.assertEquals("Orders fetched for customer Successfully",js2.get("message"));
				...
				RequestSpecification deleteOrder =given().log().all().spec(DeleteOrderReq).pathParam("productId",productId);
				
				String deleteProductResponse = deleteOrder.when().delete("api/ecom/order/delete-order/{productId}").then().log().all().
		extract().response().asString();
